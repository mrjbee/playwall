...
public class GamePadAdapter {
    ...
    void initialize() {
        ...
        if (cs.length == 0) {
            Logs.JINPUT.d("No controllers found");
            throw new GamePadDetectException("No game pad found [controllers]");
        }
        else
            Logs.JINPUT.d("Num. controllers: " + cs.length);


        // get the game pad controller
        controller = findGamePad(cs);
        Logs.JINPUT.d("Game controller: " +
                controller.getName() + ", " +
                controller.getType());
        ...
        */
    {
        ...
        if (i == cs.length) {
            Logs.JINPUT.d("No game pad found");
            throw new GamePadDetectException("No game pad found");
        }
        else
            Logs.JINPUT.d("Game pad index: " + i);
        ...
        */
    {
        ...
        if (comps.length == 0) {
            Logs.JINPUT.d("No Components found");
            throw new GamePadDetectException("No gamepad components found");
        }
        else
            Logs.JINPUT.d("Num. Components: " + comps.length);
    ...
    array index, or -1 */
    {
        ...
        for(int i=0; i < comps.length; i++) {
            ...
            if ((c.getIdentifier() == id) && !c.isRelative()) {
                Logs.JINPUT.d("Found " + c.getName() + "; index: " + i);
                return i;
            }
        }

        Logs.JINPUT.d("No " + nm + " component found");
        return -1;
     ...
     buttonsIdx[] with -1's. */
    {
        ...
        for(int i=0; i < comps.length; i++) {
            ...
            if (isButton(c)) {    // deal with a button
                if (numButtons == NUM_BUTTONS)   // already enough buttons
                    Logs.JINPUT.d("Found an extra button; index: " + i + ". Ignoring it");
                else {
        ...
        if (numButtons < NUM_BUTTONS) {
            Logs.JINPUT.d("Too few buttons (" + numButtons +
        ...
        */
    {
        if (!c.isAnalog() && !c.isRelative()) {    // digital and absolute
            ...
            // Logs.JINPUT.d(c.getName() + " identifier: " + className);
     ...
     an arbitrary decision. */
    {
        ...
        if (rumblers.length == 0) {
            Logs.JINPUT.d("No Rumblers found");
            rumblerIdx = -1;
        }
        else {
            Logs.JINPUT.d("Rumblers found: " + rumblers.length);
    ...
    // return the (x,y) analog stick compass direction
    {
        if ((xAxisIdx == -1) || (yAxisIdx == -1)) {
            Logs.JINPUT.d("(x,y) axis data unavailable");
            return NONE;
        }
        else
    ...
    // return the (z,rz) analog stick compass direction
    {
        if ((zAxisIdx == -1) || (rzAxisIdx == -1)) {
            Logs.JINPUT.d("(z,rz) axis data unavailable");
            return NONE;
        }
        else
    ...
    // Return the axes as a single compass value
    {
        ...
        // Logs.JINPUT.d("(x,y): (" + xCoord + "," + yCoord + ")");

        int xc = Math.round(xCoord);
        int yc = Math.round(yCoord);
        // Logs.JINPUT.d("Rounded (x,y): (" + xc + "," + yc + ")");
        ...
        if ((yc == -1) && (xc == -1))   // (y,x)
            ...
            Logs.JINPUT.d("Unknown (x,y): (" + xc + "," + yc + ")");
            return NONE;
        }
    ...
    {
        if (povIdx == -1) {
            Logs.JINPUT.d("POV hat data unavailable");
            return NONE;
        }
        else {
            ...
            if (povDir == POV.CENTER)  //	0.0f
                ...
                Logs.JINPUT.d("POV hat value out of range: " + povDir);
                return NONE;
            }
        }
    }  // end of getHatDir()
        ...
        */
    {
        if ((pos < 1) || (pos > NUM_BUTTONS)) {
            Logs.JINPUT.d("Button position out of range (1-" +
                    NUM_BUTTONS + "): " + pos);
            return false;
        }
